...
| **Z Floor Reference** | Sea Level (0) | Controls the elevation that maps to each piece's terrain floor. **Sea Level (0)** uses elevation 0 — pieces share a common datum and assemble at consistent relative heights. **Minimum Elevation (of DEM)** uses the lowest point across the entire DEM — all pieces share a data-anchored reference without relying on a sea-level datum. **Minimum Elevation (per piece)** uses each piece's own local minimum — maximizes visible relief per piece, but pieces are not height-comparable to one another. |
